...
class Sweep:
    ...
    def _parse_params(self) -> None:
        # Support explicit 'objects' list form to avoid unhashable keys
        objects_entries = self.params.get('objects') or self.params.get('_objects')
        if isinstance(objects_entries, list):
            for entry in objects_entries:
                if not isinstance(entry, dict) or 'targets' not in entry or 'params' not in entry:
                    raise TypeError("Each objects entry must be a dict with keys 'targets' and 'params'")
                targets = tuple(entry['targets'])
                spec_val = entry['params']
                if not isinstance(spec_val, dict):
                    raise TypeError("objects.params must be a dict of {attr: sequence}")
                norm: Dict[str, Sequence[Any]] = {}
                for attr, seq in spec_val.items():
                    if isinstance(seq, (list, tuple)):
                        norm[attr] = list(seq)
                    else:
                        norm[attr] = [seq]
                self._object_specs.append((targets, norm))

        # Parse remaining top-level entries
        for key, val in self.params.items():
            if key in ('objects', '_objects'):
                continue
