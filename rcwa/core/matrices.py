...
class MatrixCalculator:
    ...
    def _P_matrix_tensor(self):
        """P matrix calculation for tensor materials."""
        from .adapters import LayerTensorAdapter
        return LayerTensorAdapter.adapt_P_matrix_for_tensor(self, self.Kx, self.Ky)

    def _Q_matrix_tensor(self):
        """Q matrix calculation for tensor materials."""  
        from .adapters import LayerTensorAdapter
        return LayerTensorAdapter.adapt_Q_matrix_for_tensor(self, self.Kx, self.Ky)
    ...
    def VWLX_matrices(self):
        # Tensor eigensolver is opt-in to preserve legacy-tested behaviour.
        # For patterned layers (non-homogeneous), prefer the Omega^2=P@Q pathway even
        # for tensor materials, as the 4N Berreman formulation requires additional
        # normalization to produce stable scattering amplitudes across Fourier orders.
        if (
            hasattr(self, 'is_anisotropic')
            and self.is_anisotropic
            and getattr(self, '_use_tensor_eigensolver', False)
            and getattr(self, 'homogenous', True)
        ):
            return self._VWLX_matrices_tensor()
    ...
    def _VWLX_matrices_general(self):
        ...
        if self.homogenous:
            ...
            # Isotropic patterned case: classic Omega^2 = P @ Q eigenproblem
    ...
    def _VWLX_matrices_tensor(self):
        """Tensor-aware VWLX using 4N×4N eigenproblem for anisotropic media.

        Uses the LayerTensorAdapter to build tensor-coupled blocks and
        EigensolverTensorAdapter to obtain modal fields consistently.
        """
        from .adapters import LayerTensorAdapter, EigensolverTensorAdapter

        # Ensure Kx/Ky are available
        Kx = self.Kx if isinstance(self.Kx, np.ndarray) else np.array([[self.Kx]], dtype=complex)
        Ky = self.Ky if isinstance(self.Ky, np.ndarray) else np.array([[self.Ky]], dtype=complex)

        # Build tensor-coupled P/Q via adapter (also populates R/S internally)
        P = LayerTensorAdapter.adapt_P_matrix_for_tensor(self, Kx, Ky)
        Q = LayerTensorAdapter.adapt_Q_matrix_for_tensor(self, Kx, Ky)

        # If tensors are effectively diagonal (no off-diagonal coupling), we can
        # use the reduced 2N eigenproblem Omega^2 = P @ Q for improved numerical
        # robustness and compatibility with the isotropic limit.
        try:
            eps = np.array(self.tensor_material.epsilon_tensor, dtype=complex)
            mu = np.array(self.tensor_material.mu_tensor, dtype=complex)
            off_eps = eps - np.diag(np.diag(eps))
            off_mu = mu - np.diag(np.diag(mu))
            if np.allclose(off_eps, 0.0, atol=1e-12) and np.allclose(off_mu, 0.0, atol=1e-12):
                OmegaSquared = omega_squared_matrix(P, Q)
                eigenValues, W = eig(OmegaSquared)
                Lambda = np.diag(sqrt(eigenValues))
                # Safe inverse of Lambda
                lam = np.diag(Lambda)
                lam_inv = np.zeros_like(lam)
                sel = np.abs(lam) > 1e-15
                lam_inv[sel] = 1.0 / lam[sel]
                LambdaInverse = np.diag(lam_inv)
                V = Q @ W @ LambdaInverse
                X = matrixExponentiate(-Lambda * self.source.k0 * self.thickness)
                return (V, W, Lambda, X)
        except Exception:
            # Fall back to full tensor path on any issues
            pass

        # Solve full tensor eigenproblem to obtain modal fields. Use electric
        # partition (W) from eigensolver, then enforce the constitutive
        # relation V = Q @ W @ Lambda^{-1} to remain consistent with the
        # scattering formulation used downstream.
        _, W, Lambda_raw, _V_unused = EigensolverTensorAdapter.solve_tensor_eigenproblem(self, P, Q)

        # The first-order Berreman system is d/dz f = i*k0*M*f. The eigensolver
        # above returns eigenvalues of M. Therefore the physical propagation
        # constants are i * eigen(M). Incorporate that i here to ensure unitary
        # propagation for lossless media and correct phase accumulation.
        lam_m = np.diag(Lambda_raw)
        lam_phys = 1j * lam_m
        Lambda = np.diag(lam_phys)

        # Safe inverse of physical Lambda
        lam_inv_diag = np.zeros_like(lam_phys)
        nz = np.abs(lam_phys) > 1e-15
        lam_inv_diag[nz] = 1.0 / lam_phys[nz]
        LambdaInverse = np.diag(lam_inv_diag)
    ...
    def _S_matrix_reflection(self):
        # Tensor eigensolver is opt-in
        if hasattr(self, 'is_anisotropic') and self.is_anisotropic and getattr(self, '_use_tensor_eigensolver', False):
            return self._S_matrix_reflection_tensor()
        if isinstance(self.Kx, np.ndarray):
    ...
    def _S_matrix_reflection_tensor(self):
        """Reflection S-matrix using tensor-aware eigenmodes."""
        (Vi, Wi, _, _) = self.VWLX_matrices()
        Ai = A_matrix(self.Wg, Wi, self.Vg, Vi)
        Bi = B_matrix(self.Wg, Wi, self.Vg, Vi)
        return calculateReflectionRegionSMatrixFromRaw(Ai, Bi)
    ...
    def _S_matrix_transmission(self):
        # Tensor eigensolver is opt-in
        if hasattr(self, 'is_anisotropic') and self.is_anisotropic and getattr(self, '_use_tensor_eigensolver', False):
            return self._S_matrix_transmission_tensor()
        if isinstance(self.Kx, np.ndarray):
    ...
    def _S_matrix_transmission_tensor(self):
        """Transmission S-matrix using tensor-aware eigenmodes."""
        (Vi, Wi, _, _) = self.VWLX_matrices()
        Ai = A_matrix(self.Wg, Wi, self.Vg, Vi)
        Bi = B_matrix(self.Wg, Wi, self.Vg, Vi)
        return calculateTransmissionRegionSMatrixFromRaw(Ai, Bi)
