...
class LayerTensorAdapter:
    ...
    def _compute_tensor_blocks(layer, Kx: ArrayLike, Ky: ArrayLike) -> Dict[str, ArrayLike]:
        ...
        # Prefer inverse-rule convolution matrices when available for better physics
        inv_ezz_key = 'inv_er_zz'
        inv_mzz_key = 'inv_ur_zz'
        if inv_ezz_key in conv:
            eps_zz_inv = _as_square(conv[inv_ezz_key], target_size)
        else:
            eps_zz_inv = _safe_inverse(ezz)
        if inv_mzz_key in conv:
            mu_zz_inv = _as_square(conv[inv_mzz_key], target_size)
        else:
...
class EigensolverTensorAdapter:
    ...
    def solve_tensor_eigenproblem(layer, P: ArrayLike, Q: ArrayLike) -> Tuple[ArrayLike, ArrayLike, ArrayLike, ArrayLike]:
        ...
        # keep those with positive imaginary propagation constants for the
        # physical Lambda = i * eigen(system_matrix) used in propagation. Since
        # Lambda_phys = i * eigenValues, Im(Lambda_phys) = Re(eigenValues).
        real_vals = np.real(eigenValues)
        tol = 1e-9

        pos_idx = np.where(real_vals > tol)[0]
        zero_idx = np.where(np.abs(real_vals) <= tol)[0]
        neg_idx = np.where(real_vals < -tol)[0]

        # Sort each group by descending imaginary part magnitude to pick the most
        # physically relevant modes first.
        pos_sorted = pos_idx[np.argsort(real_vals[pos_idx])[::-1]] if pos_idx.size else np.array([], dtype=int)
        zero_sorted = zero_idx[np.argsort(np.abs(eigenValues[zero_idx]))] if zero_idx.size else np.array([], dtype=int)
        neg_sorted = neg_idx[np.argsort(np.abs(real_vals[neg_idx]))] if neg_idx.size else np.array([], dtype=int)

        # Ensure we capture both polarisation branches when Kx=Ky=0 by
        # prioritising unique magnitude pairs. This avoids selecting only the
        # smallest-magnitude branch in highly symmetric cases.
