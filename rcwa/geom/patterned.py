...
class PatternedLayer(Layer):
    ...
    # Generate cache key
        ...
        # Also build convolution matrices for inverse normal components (Li inverse rule)
        # These are useful for more physically correct elimination of normal fields.
        # Compute 1/epsilon_zz and 1/mu_zz in real space then FFT to convolution matrices.
        # Guard against division by zero by using a tiny floor.
        eps_zz_field = epsilon_field[:, :, 2, 2]
        mu_zz_field = mu_field[:, :, 2, 2]
        tiny = 1e-12
        inv_eps_zz = 1.0 / np.where(np.abs(eps_zz_field) > tiny, eps_zz_field, tiny)
        inv_mu_zz = 1.0 / np.where(np.abs(mu_zz_field) > tiny, mu_zz_field, tiny)

        # FFT and build convolution matrices for the reciprocals
        inv_eps_fft = np.fft.fftshift(np.fft.fft2(inv_eps_zz))
        inv_mu_fft = np.fft.fftshift(np.fft.fft2(inv_mu_zz))

        conv_matrices['inv_er_zz'] = self._build_convolution_matrix(
            inv_eps_fft, (Ny, Nx), (Nh_y, Nh_x)
        )
        conv_matrices['inv_ur_zz'] = self._build_convolution_matrix(
            inv_mu_fft, (Ny, Nx), (Nh_y, Nh_x)
        )
