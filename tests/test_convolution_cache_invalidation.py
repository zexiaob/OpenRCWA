...
def test_cache_changes_when_rotation_changes():
    ...
    assert key1 != key2
    # Ensure separate caches per instance and key
    assert layer.get_cache_info()['cache_size'] == 1
    assert layer_r.get_cache_info()['cache_size'] == 1
...
def test_cache_changes_when_lattice_changes():
    ...
    assert key1 != key2
    # Caches are per instance; each should have one entry
    assert layer.get_cache_info()['cache_size'] == 1
    assert layer2.get_cache_info()['cache_size'] == 1
...
def test_cache_changes_when_resolution_changes():
    ...
    assert key1 != key2
    assert layer.get_cache_info()['cache_size'] == 1
    assert layer_hi.get_cache_info()['cache_size'] == 1
