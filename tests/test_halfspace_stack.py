...
def test_air_factory():
    """Test Air factory function."""
    air_material = Air()
    air_halfspace = HalfSpace(air_material)
    
    assert isinstance(air_material, Material)
    assert isinstance(air_halfspace, HalfSpace) 
    assert air_material.er == 1.0
    assert air_material.ur == 1.0
    assert air_material.n == 1.0
    assert air_halfspace.is_anisotropic == False
...
def test_halfspace_string_representation():
    ...
    # Isotropic halfspace
    air_halfspace = HalfSpace(Air())
    air_str = str(air_halfspace)
