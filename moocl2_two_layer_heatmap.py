"""Sweep a two-layer MoOCl2 stack over rotation angle and wavelength, producing heatmaps.

This script extends the single-layer example to a bilayer structure where the
second layer is rotated about the optical axis by an angle \theta. For each
angle and wavelength, the code records total (LCP/RCP) transmission and
reflection, exports the data to CSV, and saves heatmaps visualising the
...
def build_two_layer_stack(theta_rad: float, thicknesses: Tuple[float, float]) -> Stack:
    ...
    rotation = rotation_matrix_z(theta_rad)
    rotated_material = base_material.rotated(rotation)
    layer2 = Layer(tensor_material=rotated_material, thickness=thicknesses[1])

    air = Air()
    stack = Stack(layers=[layer1, layer2], superstrate=air, substrate=air)
...
def run_sweep(
    ...
    thicknesses_nm: Tuple[float, float],
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    theta_rad = np.deg2rad(theta_deg)
    wavelengths_m = wavelengths_nm * 1e-9
    thicknesses_m = (nm(thicknesses_nm[0]), nm(thicknesses_nm[1]))
    ...
    for idx, angle in enumerate(theta_rad):
        stack = build_two_layer_stack(angle, thicknesses_m)
        grid = simulate(
            stack,
...
def plot_cd(
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        denom_T = lcp_T + rcp_T
        denom_R = lcp_R + rcp_R
        cd_T = np.divide(lcp_T - rcp_T, denom_T, out=np.zeros_like(denom_T, dtype=float), where=~np.isclose(denom_T, 0.0))
        cd_R = np.divide(lcp_R - rcp_R, denom_R, out=np.zeros_like(denom_R, dtype=float), where=~np.isclose(denom_R, 0.0))
...
def main(
    *,
    ...
    wavelengths_nm = np.linspace(400.0, 600.0, 201)

    lcp_T, rcp_T, lcp_R, rcp_R = run_sweep(
        theta_deg,
        wavelengths_nm,
        (thickness_layer1_nm, thickness_layer2_nm),
    )

    csv_path = repo_root / "moocl2_two_layer_rt.csv"
    save_results(wavelengths_nm, theta_deg, lcp_T, rcp_T, lcp_R, rcp_R, csv_path)

    if save_plots:
        heatmap_path = repo_root / "moocl2_two_layer_TR_heatmaps.png"
        ...
        cd_path = repo_root / "moocl2_two_layer_cd_heatmap.png"
...
def parse_args():
    parser = ArgumentParser(description="MoOCl2 two-layer rotation sweep")
