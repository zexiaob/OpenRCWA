...
class PatternedLayer:
    ...
    def rasterize_tensor_field(self, wavelength: Optional[float] = None) -> np.ndarray:
        """
        Rasterize the pattern to a 2D tensor field.
        
        Creates a grid of epsilon tensors representing the material distribution.
        Each pixel contains a full 3x3 complex tensor supporting anisotropy.
        
        :param wavelength: Wavelength for dispersive materials (meters)
        :return: Array of shape (Ny, Nx, 3, 3) with epsilon tensors
        """
        ...
        # Initialize with background material tensor
        tensor_field = np.zeros((Ny, Nx, 3, 3), dtype=complex)
        
        # Get background tensor
        bg_tensor = self._get_material_tensor(self.background_material, wavelength)
        tensor_field[:] = bg_tensor
        ...
        for shape in self.shapes:
            if shape.material is not None:
                ...
                if np.any(mask):
                    # Get shape material tensor
                    shape_tensor = self._get_material_tensor(shape.material, wavelength)
                    tensor_field[mask] = shape_tensor
        
        return tensor_field
    
    def _get_material_tensor(self, material: Union[Material, TensorMaterial], 
                           wavelength: Optional[float]) -> np.ndarray:
        """
        Extract epsilon tensor from material.
        
        :param material: Material object
        :param wavelength: Wavelength for dispersive materials
        :return: 3x3 complex tensor
        """
        if wavelength is None:
            wavelength = 550e-9  # Default wavelength
        
        if isinstance(material, TensorMaterial):
            # TensorMaterial - get full tensor
            if callable(material.epsilon_tensor):
                return material.epsilon_tensor(wavelength)
            else:
                return material.epsilon_tensor
        else:
            # Regular Material - convert to diagonal tensor
            if callable(material.er):
                er = material.er(wavelength)
            else:
                er = material.er
            
            return np.eye(3, dtype=complex) * er
    ...
    def to_convolution_matrices(self, harmonics: Tuple[int, int], 
        ...
        # Rasterize tensor field
        tensor_field = self.rasterize_tensor_field(wavelength)
        Ny, Nx = tensor_field.shape[:2]
        
        # Extract harmonic range
        Nh_x, Nh_y = harmonics
        
        # Initialize convolution matrices
        conv_matrices = {}
        
        # Component names for full tensor
        tensor_components = [
        ...
        for comp_name, i, j in tensor_components:
            ...
            component_field = tensor_field[:, :, i, j]
            
            # Compute 2D FFT
            fft_field = np.fft.fft2(component_field)
            fft_field = np.fft.fftshift(fft_field)
            
            # Extract harmonics and build convolution matrix
            conv_matrix = self._build_convolution_matrix(
                fft_field, (Ny, Nx), (Nh_y, Nh_x)
            )
            
            conv_matrices[f'eps_{comp_name}'] = conv_matrix
    ...
    def _generate_cache_key(self, harmonics: Tuple[int, int], 
        ...
        cache_dict = {
            ...
            'shapes': [shape.get_hash() for shape in self.shapes],
