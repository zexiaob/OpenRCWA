...
class TestTensorMaterialValidation:
    ...
    def test_dispersive_source_warning(self):
        ...
        # No warning when source is provided (check by counting warnings)
        with pytest.warns(None) as warning_list:
            mat = TensorMaterial(epsilon_tensor=eps_func, source="dummy_source")
            # Check no warnings were raised about source
            source_warnings = [w for w in warning_list.list if "source" in str(w.message)]
    ...
    def test_backward_compatibility(self):
        ...
        mat = TensorMaterial(epsilon_tensor=eps_func)
        # For dispersive tensors, epsilon_tensor is callable
