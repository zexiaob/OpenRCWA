...
class TestSolver(unittest.TestCase):
    ...
    def testIntegrationMultiWavelength(self):
        """Test solver with multiple wavelengths for integration verification."""
        testWavelengths = self.solver.source.wavelength*np.arange(0.2,2,0.1)  # Reduced range for faster testing
        self.solver.solve(wavelength=testWavelengths)
        
        # Verify that results contain multiple wavelengths
        assert 'wavelength' in self.solver.results.keys(), "Results should contain wavelength data"
        assert len(self.solver.results['wavelength']) == len(testWavelengths), "Number of wavelengths should match input"
        
        # Verify that all essential quantities are present
        required_keys = ['rx', 'ry', 'rz', 'tx', 'ty', 'tz', 'R', 'T', 'RTot', 'TTot', 'conservation']
        for key in required_keys:
            assert key in self.solver.results.keys(), f"Results should contain {key}"
            if key in ['RTot', 'TTot', 'conservation']:
                assert len(self.solver.results[key]) == len(testWavelengths), f"{key} should have same length as wavelengths"
        
        # Verify energy conservation for all wavelengths
        for i, wl in enumerate(testWavelengths):
            conservation = self.solver.results['conservation'][i]
            assert abs(conservation - 1.0) < 0.01, f"Energy conservation violated at wavelength {wl}: {conservation}"
        
        # Verify that complex amplitudes are present and have correct dimensions
        complex_keys = ['rx', 'ry', 'rz', 'tx', 'ty', 'tz']
        for key in complex_keys:
            amplitude_data = self.solver.results[key]
            assert len(amplitude_data) == len(testWavelengths), f"{key} should have same length as wavelengths"
            # Each amplitude should be a complex array with proper shape
            assert isinstance(amplitude_data[0], (complex, np.ndarray)), f"{key} should contain complex data"
