...
def test_geometry_workflow_integration():
    ...
    # Step 3: Create PatternedLayer (which is directly a Layer!)
    ...
    assert not patterned_layer.homogenous  # It's patterned
    ...
    # Step 6: Test convolution matrix generation (native Layer interface)
    ...
    assert conv_matrix.shape == (49, 49)  # 7x7 harmonics = 49 total harmonics, so 49x49 matrix
    assert np.all(np.isfinite(conv_matrix))
    
    # Step 7: Test mixed stack with regular layers (no conversion needed!)
    regular_layer1 = Layer(material=Material(er=2.25, ur=1.0), thickness=0.1)  # SiO2-like
    regular_layer2 = Layer(material=Material(er=16.0, ur=1.0), thickness=0.05)  # High-index
    
    mixed_stack = [regular_layer1, patterned_layer, regular_layer2]
    
    # All are Layers - no conversion required!
    for layer in mixed_stack:
        assert isinstance(layer, Layer)
    
    assert len(mixed_stack) == 3
    assert mixed_stack[0].homogenous  # Regular layer
    assert not mixed_stack[1].homogenous  # Patterned layer
    assert mixed_stack[2].homogenous  # Regular layer
    
    # Step 8: Test automatic harmonics suggestion
...
def test_parametric_geometry_integration():
    ...
    # Create simple parametric geometry (simplified test for now)
    air = Material(er=1.0, ur=1.0)
    silicon = Material(er=12.0, ur=1.0)

    # Create base shapes with different sizes (simulating parameter sweep)
    circle1 = Circle(center=(0.25, 0.25), radius=0.1)
    circle2 = Circle(center=(0.25, 0.25), radius=0.2)
    circle3 = Circle(center=(0.25, 0.25), radius=0.3)
    
    # Create different layers for different parameter values
    layer1 = PatternedLayer(
        thickness=0.2,
        lattice=((1.0, 0.0), (0.0, 1.0)),
        background_material=air,
        shapes=[(circle1, silicon)]
    )
    ...
    layer2 = PatternedLayer(
        ...
        lattice=((1.0, 0.0), (0.0, 1.0)),
        background_material=air,
        shapes=[(circle2, silicon)]
    )
    
    layer3 = PatternedLayer(
        thickness=0.2,
        lattice=((1.0, 0.0), (0.0, 1.0)),
        background_material=air,
        shapes=[(circle3, silicon)]
    )
    
    layers = [layer1, layer2, layer3]
    
    # Verify parameter changes (check radius differences)
    for i, layer in enumerate(layers):
        circle_shape = layer.shapes[0][0]
        expected_radius = 0.1 + 0.1 * i  # 0.1, 0.2, 0.3
        
        assert np.isclose(circle_shape.radius, expected_radius)
        
        # Test that they're all valid PatternedLayers (which are Layers)
        from rcwa.model.layer import Layer
        assert isinstance(layer, Layer)
        assert not layer.homogenous  # Patterned layers are not homogeneous
    
    # Test rasterization for different layers
    for layer in layers:
        er_field, ur_field = layer.rasterize_tensor_field(wavelength=1.5e-6)
        assert er_field.shape == (256, 256)
        assert ur_field.shape == (256, 256)
        assert np.any(np.isclose(er_field, 1.0))   # Air background
        assert np.any(np.isclose(er_field, 12.0))  # Silicon features
    
    # Calculate silicon fraction for each layer to verify different fill factors
    fields = []
    for layer in layers:
